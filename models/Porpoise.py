...
class PorpoiseMMF(nn.Module):
    def __init__(self, 
        ...
        if dropinput:
            fc_omic = [nn.Dropout(dropinput), nn.Linear(size[0], size[1]), nn.ReLU(), nn.Dropout(dropout)]
        else:
            fc_omic = [nn.Linear(size[0], size[1]), nn.ReLU(), nn.Dropout(dropout)]
    ...
    def forward(self, **kwargs):
        ...
        logits  = self.classifier_mm(h_mm) # logits needs to be a [B x 4] vector      
        # assert len(h_mm.shape) == 2 and h_mm.shape[1] == self.n_classes
        Y_hat = torch.argmax(logits, dim=1)
        Y_prob = F.softmax(logits, dim = 1)
        return logits, Y_prob, Y_hat, h_mm
