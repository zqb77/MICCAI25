...
class WiKG(nn.Module):
    def __init__(self, dim_in=1024, dim_hidden=256, topk=6, n_classes=2, agg_type='bi-interaction', dropout=0.3, pool='attn'):
        super().__init__()
    ...
    def forward(self, **kwargs):
        x  = kwargs['x_path'].unsqueeze(0)  # [B,N,C]
        x = self._fc1(x)    # [B,N,C]
...
if __name__ == "__main__":
    data = torch.randn((1, 10000, 1024)).cuda()
    model = WiKG(dim_in=1024, dim_hidden=256, topk=6, n_classes=2, agg_type='bi-interaction', dropout=0.3, pool='attn').cuda()
    output = model(x_path=data)
