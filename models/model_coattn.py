...
class MCAT_Surv(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        h_omic_bag = self.snn(x_omic).unsqueeze(1) ### omic embeddings are fed through a snn layer


        # Coattn
        # h_path_coattn, _, h_omic_bag, _= self.coattn(h_path_bag, h_omic_bag)
        ...
        if self.fusion == 'bilinear':
            h = self.mm(h_path.unsqueeze(dim=0), h_omic.unsqueeze(dim=0)).view(-1, 256)
        elif self.fusion == 'concat':
            h = self.mm(torch.cat([h_path, h_omic], axis=0)).view(-1, 256)
                
        logits = self.classifier(h) ## K x num_cls
        Y_hat = torch.argmax(logits, dim=1)
        Y_prob = F.softmax(logits, dim=1)
        
        return  logits, Y_prob, Y_hat, h
